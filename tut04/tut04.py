...
#octant names, length, frequency and end time of longest subsequence
data = {
    ...
    for i in range(len(df) - 1):
        if df["Octant"][i] == df["Octant"][i+1]:
            ...
            if l_of_string == data["length_of_longest_s"][str(df["Octant"][i])]:
                ...
                data["end_time"][str(df["Octant"][i])].append(df["Time"][i+1])
...
#time range of longest subsequence
    df.iloc[9, 12] = "Count"
